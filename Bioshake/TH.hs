...
makeSingleCluster ty tags fun = do
  ...
  consName <- newName name'
  constructorSig <- return $ SigD consName (ForallT [] [AppT (ConT ''Given) (ConT ''Config)] funType)
  constructor <- return $ ValD (VarP consName) (NormalB (AppE (ConE con) (VarE 'given))) []
...
makeThreaded ty tags fun = do
  ...
  consName <- newName name'
  constructorSig <- return $ SigD consName (ForallT [] [AppT (ConT ''Given) (ConT ''Threads)] funType)
  constructor <- return $ ValD (VarP consName) (NormalB (AppE (ConE con) (VarE 'given))) []
...
withCmd :: Given Resource => Int -> Cmd () -> Action ()
withCmd t x' = do
    x <- runFreeT x'
    case x of
        Pure _ -> return ()
        Free (CmdF str a) -> do
            () <- withResource given t $ cmd Shell str
            withCmd t a
